...
// Changeable code continues here. 
	...
	int main() {
		...
		int n, h, m;
		while (cin >> n >> h >> m) {
			vector<pair<int, int>> gist(n, make_pair(0, h + 1));
			for (int i = 0, x, y, e; i < m; ++i) {
				cin >> x >> y >> e;
				--x;
				if (e)
					gist[x].first = max(gist[x].first, y);
				else
					gist[x].second = min(gist[x].second, y);
			}
			ull result = 1;
			bool possible = true;
			for (auto p : gist) {
				...
				result *= (ull)(p.second - p.first);
			}
			cout << (possible ? result : 0) << '\n';
		}
