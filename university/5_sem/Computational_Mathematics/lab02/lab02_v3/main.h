	...
	int main() {
		...
		while (cin >> method >> taskType >> matrixSize) {
			...
			if (method == GaussSeidel) {
				...
				if (!consistent) {
					cout << "Matrix is not consistent, cannot find solution\n\n";
					continue;
				}
				...
				if (taskType == SLAE) {
					...
					RealMatrix x(matrixSize, matrixSize, 0.0);
					//RealMatrix a(matrixSize, matrixSize);
					//RealMatrix c(matrixSize, matrixSize);
					//RealMatrix d(matrixSize, matrixSize);
					//
					//for (size_t i = 0; i < matrixSize; ++i) {
					//	for (size_t j = 0; j < matrixSize; ++j) {
					//		a[i][j] = matrix[i][j];
					//		c[i][j] = a[i][j] / a[j][j];
					//		d[i][j] = i == j ? 1.0 / a[j][j] : 0.0;
					//	}
					//}
					...
					for (; ; ++step) {
						...
						// x = DI * (L * x + U * prevX + identityMatrix);

						x = (1 - lambda) * prevX + lambda * DI * (L * x + U * prevX + identityMatrix);

						RealMatrix residue = identityMatrix - matrix * x;

						double determResidue = getDeterminant(residue);
					...
					RealMatrix residualMatrix = initialMatrix * x - identityMatrix;
					RealMatrixStatementOutput residualMatrixOutput(matrixSize);
					residualMatrixOutput << formatWidth(6)
						<< &initialMatrix << " *" << &x << " -" << &identityMatrix
						<< " =" << &residualMatrix;
					cout << "Residual matrix:\n" << residualMatrixOutput << "\n\n";
