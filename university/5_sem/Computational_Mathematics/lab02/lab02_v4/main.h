...
// Changeable code continues here. 
	...
	template<class Func>
	bool has_func(const RealMatrix& matrix, const Func& func) {
	...
	double norm(const RealMatrix& matrix, int p) {
		double result = 0.0;
		for (auto iter = matrix.colCBegin(); iter != matrix.colCEnd(); ++iter)
			for (auto el : iter)
				result += pow(fabs(el), p);
		return pow(result, 1.0 / (double)p);
	}
		...
		while (cin >> method >> taskType >> matrixSize) {
			...
			if (method == GaussSeidel) {
				...
				case SLAE:
					...
					break;
				case Determinant:
					cout << "Determinant cannot be found using this method\n\n";
					continue;
				case Invertible:
					cout << "....................  Matrix inversion  ....................\n\n";
					break;
				default:
					cout << "Task type is incorrect\n\n";
					continue;
				}

				RealMatrix L(matrixSize, matrixSize, 0.0);
				RealMatrix U(matrixSize, matrixSize, 0.0);
				RealMatrix DI(matrixSize, matrixSize, 0.0);
				RealMatrix identityMatrix(matrixSize);

				for (size_t i = 0; i < matrixSize; ++i) {
					for (size_t j = 0; j < matrixSize; ++j) {
						if (i == j)
							DI[i][j] = 1.0 / matrix[i][j];
						else if (i < j)
							L[i][j] = matrix[i][j];
						else
							U[i][j] = matrix[i][j];
				...
				RealMatrix H = RealMatrix(matrixSize, matrixSize, 0.0) - (DI * L + DI * U);

				output.clear();
				output << H;
				cout << "H matrix is:\n" << output << "\n";

				double h_norm = norm(H, 2);
				cout << "Norm of H matrix is: " << h_norm << "\n\n";
				...
				if (taskType == SLAE) {
					...
					for (; ; ++step) {
						...
						debug << output << '\n';
						if (norm(xM - prevXM, 2) <= EPS || step > 10000)
							break;
				...
				else { // taskType == Invertible

					RealMatrix result(matrixSize, matrixSize);

					for (size_t i = 0; i < matrixSize; ++i) {

						RealMatrix x(1, matrixSize, 0.0);
						RealMatrix prevX = x;

						RealMatrix b(1, matrixSize, 0.0);
						b[0][i] = 1.0 / matrix[i][i];

						//static const double lambda = 0.628;
						...
						for (; ; ++step) {
							...
							for (size_t i = 0; i < matrixSize; ++i) {
								...
								for (size_t j = i; j < matrixSize; ++j)
									x[0][i] += H[j][i] * prevX[0][j];

							}

							//x = (1.0 - lambda) * prevX + lambda * DI * (L * x + U * prevX + b);
							//x = DI * (L * x + U * prevX + b);

							if (has_func(x, isnan<double>) ||
								has_func(x, isinf<double>) ||
								norm(x - prevX, 2) <= EPS ||
								step > 10000)
								break;
					...
					output.clear();
					output << &result;
					cout << "Invertible matrix is:\n" << output << "\n";

					RealMatrix residualMatrix = initialMatrix * result - identityMatrix;
					RealMatrixStatementOutput residualMatrixOutput(matrixSize);
					residualMatrixOutput << formatWidth(6)
						<< &initialMatrix << " *" << &result << " -" << &identityMatrix
						<< " =" << &residualMatrix;
					cout << "Residual matrix:\n" << residualMatrixOutput << "\n\n";
