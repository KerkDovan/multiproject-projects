...
#define exit_program(msg) { \
	...
	int main(int argc, char** argv) {
		...
		int MAC;
		if (argc <= 2) {
			cout << "Command line arguments should be: MSAU_PORT PC_MAC" << endl;
			port = 1337;
			MAC = 322;
		}
		else {
			port = StrToNum<PortType>(argv[1]);
			MAC = StrToNum<int>(argv[2]);
		}
...
#define end_work \
		...
		// Trying to register at MSAU
		cout << "Trying to register at MSAU" << endl;
		buffer = "00";
		pc.msau.send(buffer);
		recieved = pc.msau.recieve(buffer);
		if (recieved < 0 || buffer[1] == '0')
			exit_program("Cannot register at MSAU");
		cout << "Registered at MSAU" << endl;
